class Game < ActiveRecord::Base
  ...
  def correct_places
    if @@round >= 2 && @@color_array.length >= 1
      if @correct_color == "0"
        if @correct_place >= "1"
  ...
  #update database with new guess
  def update
    update_attributes(color1: @@guess[-1][-4], color2: @@guess[-1][-3].to_s, color3: @@guess[-1][-2], color4: @@guess[-1][-1])
  end
